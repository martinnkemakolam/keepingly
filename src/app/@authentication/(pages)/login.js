...
export default function SignIn({ setCookies }) {
  ...
  let [passError, passMsg, setPassError, passChange] = ErrorHook();
    ...
    >
      ...
      <Input
        ...
        changeFunc={(name, value) =>
          handleInput(name, value, loginDetail, setLoginDetails)
        }
      ...
      <div className={style.checkBox}>
        ...
        <Link className={style.colRed} href={`/authentication/forgot`}>
          Forgot password?
        </Link>
      </div>
      <BottomComp
        func={() => null}
        buttonText={loadingState ? `Loading...` : `Log in`}
        col={loadingState}
        pText={`Don’t have an account yet?`}
        linkHref={`/authentication/signUp-realtor`}
        linkHref2={`/authentication/signUp-homeowner`}
        linkText2={`Register as homeowner`}
        linkText={`Register as realtor`}
      />
    </form>
  );
}
