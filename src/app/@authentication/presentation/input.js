    ...
    return(
        <div className={style.inputDiv}>
            <div className={style.error}>
                ...
                <input required={true} name={name} className={errorState ? style.colRed : ''} onChange={({target: {value}})=> changeFunc(name, value)} placeholder={inputText} type={ password ? 'password' : "email"} pattern={ password?"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$" :"/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/"} />
            </div>
