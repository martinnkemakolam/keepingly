...
export let DocumentPage =({query})=>{
    ...
    useEffect(()=>{
        ...
        getProperties()
        role.current = sessionStorage.getItem('kpur')
    },[query])
    return (
        <div className={style.video}>
            ...
            <PropertyInfo h1Text={`${properties[id]?.address_one}, ${properties[id]?.city} ${properties[id]?.zipcode}`} setId={setId} arrProperties={properties} showSvg={true} buttonText={ /* role.current === 'realtor' */ true ? (/* properties[id]?.has_received_invite */ false ? null : `Invite homeowner`) : null} buttonFunc={()=> send()} extraButtonText={`Add document`} extraButtonFunc={()=> add()}/>
