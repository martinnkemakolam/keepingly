...
export let DocumentPage =({query})=>{
    ...
    useEffect(()=>{
        async function getProperties(){
            ...
            try {
                ...
                console.log('this is',propetiiesJson)
                setProperties(propetiiesJson || [])
                query.propId && propetiiesJson.forEach((element, id) => {
                    if (element.id === query.propId) {
                        setId(id)
                    }                    
                });
            } catch (error) {
    ...
    return (
        <div className={style.video}>
            {console.log(sessionStorage.getItem('kpur'))}
            ...
            <div className={style.pill}>
                ...
                Documents added across Expenses and Maintenance will be visible here as video categories.
                </p>
                {console.log(properties[id]?.id)}
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <PropertyInfo h1Text={`${properties[id]?.address_one}, ${properties[id]?.city} ${properties[id]?.zipcode}`} setId={setId} arrProperties={properties} showSvg={true} buttonText={ sessionStorage.getItem('kpur') === 'realtor' ? ( properties[id]?.has_received_invite ? null : `Invite homeowner`) : null} buttonFunc={()=> send()} extraButtonText={`Add document`} extraButtonFunc={()=> add()}/>
            ...
            { showOverLay.bol &&
                <Overlay propId={properties[id]?.id} overlay={showOverLay.type} select={select} currentPage={showOverLay.value} closeFunc={close} type={`document`}/>
            }
        </div>
    )
}