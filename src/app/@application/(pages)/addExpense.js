...
export let AddExpense = ({ id, edit }) => {
  ...
  let submit = async () => {
    ...
    let currentType = exp.data.filter(
      (element) =>
        element.name.toLowerCase() ===
        (id
          ? Object?.values(id)[1].toLowerCase()
          : "" ||
            formData?.expense_type?.toLowerCase() ||
            edit.expense_type.name.toLowerCase())
    );
    ...
    if (id) {
      ...
      let addExpense = await api.post(
        `/api/v2/expense/${propId[0]}/`,
        dataToSend,
        {
          headers: {
            "Content-type": "application/json",
  ...
  return (
    <>
      ...
      <TopBar
        ...
        btnFunc1={() => submit()}
        buttonText1={`Save expense`}
        showSearch={true}
      />
      <PropertyInfo h1Text={`Add an expense`} />
      <ExpenseForm
        handleInput={(name, value) =>
          handleInput(name, value, formData, setFormData)
        }
        value={
          edit
            ? {
