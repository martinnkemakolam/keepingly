...
export let Expenses=(params)=>{
    ...
    return (
        ...
        <ExpenseTitle title={`Expenses`} showCount={exp.filter((ele)=> ele.property_id.id === properties[id]?.id && ele).length}  btnStyle1={`clearBtn`} btnStyle2={`clearBtn`} svg1={filterSvg} buttonText1={`Filters`} svg2={exportSvg} buttonText2={`Export as`}/>
        <ExpensesTable data={sec(exp.filter((ele)=> ele.property_id.id === properties[id]?.id && ele), step * currentPage - step, step * currentPage)} />
        <ExpenseFooter step={step} preFunc={prevPageFun} nextFunc={nextPageFun} currentPage={currentPage} lastPage={lastPage(exp, step)}/>
        </>
    )
}
