...
export let ExpenseFooter=({currentPage, lastPage, step, preFunc, nextFunc})=>{
    ...
    return(
        <div className={style.footer}>
            <div className={style.footerRow}>
                ...
                <span className={style.span}>{step}</span>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M7 10L12 15L17 10" stroke="#333333" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
            </div>
            <ClearBtn btnStyle={`clearBtnReverse`} btnFunc={()=> preFunc()} text={`Previous`}  svg={arrowLeft} />
            <p className={style.span}>
                {currentPage} of {lastPage}
            </p>
            <ClearBtn btnStyle={`clearBtn`} btnFunc={()=> nextFunc()} text={`Next`} svg={arrowRight} />
        </div>
    )
}